...
class SingleFluxTower():    
  def __init__(self, flux_tower_fname):
    ...
    harmonizing_mask = np.isnan(self._gpp) | np.isnan(self._reco) | np.isnan(self._nee)
    for tower_var in self._tower_vars:
      tower_var[harmonizing_mask] = np.nan
    self._non_missing_observations = len(harmonizing_mask) - np.count_nonzero(harmonizing_mask)
